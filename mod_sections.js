...
function renderModSection(product, _modBody, _modTemp, _modElementsTemp) {
  ...
  const originalSectionTemplate = _modElementsTemp.querySelector(
    `[data-type="section"]`
  );
  const originalTxtElement = _modElementsTemp
    .querySelector(`[data-type="text"]`)
    .cloneNode(true);
  const originalImgElement = _modElementsTemp
    .querySelector(`[data-type="img"]`)
    .cloneNode(true);
  const originalTableElementTemp = _modElementsTemp
    .querySelector(`[data-type="size-table"]`)
    .cloneNode(true);
  const originalColContainer = _modElementsTemp
    .querySelector(`[data-type="col-container"]`)
    .cloneNode(true);
  ...
  Object.entries(sectionsobj).forEach(([section, secData], index) => {
    ...
    Object.entries(secData).forEach(([col, val], i) => {
      ...
      if (currentCol.type === "text") {
        const textElem = createTxtCol(
          colSize,
          currentCol.details,
          txtElement.cloneNode(true)
        );
        colContainer.appendChild(textElem);
      }
      if (currentCol.type === "img") {
        const imgElem = createImgCol(
          colSize,
          currentCol.details,
      ...
      if (currentCol.type === "sizeTable") {
        const table = createTable(
          colSize,
          product.sizeTable,
          tableElementTemp.cloneNode(true)
        );
